...
plotFactorsMIRIMFA <- function (object,
                                comp = c(1, 2),
                                num_datasets = NULL,
                                title = NULL) {
  ## Setting functions ################################################
  ...
  ## Coordinates of Supplementaries
  ...
  ## Plot MFA while it is still buggy

  plotCorrected <- function (x,
                             axes = c(1, 2),
                             choix = c("ind",
                                       "var",
                                       "group",
                             ...
                             ggoptions = NULL, ...)
  {
    res.mfa <- x
    argument <- list(...)
    if (!is.null(argument[["cex"]]) & is.null(ggoptions["size"]))
      ggoptions["size"] <- 4 * argument$cex
    ...
    ggoptions_default <- list(size = 4, point.shape = 19, line.lty = 2,
                              line.lwd = 0.5, line.color = "black", segment.lty = 1,
                              segment.lwd = 0.5, circle.lty = 1, circle.lwd = 0.5,
                              circle.color = "black", low.col.quanti = "blue", high.col.quanti = "red3")
    if (!is.null(ggoptions[1]))
      ggoptions_default[names(ggoptions)] <- ggoptions[names(ggoptions)]
    invisible <- match.arg(invisible, c("none", "ind", "ind.sup",
                                        "quanti", "quanti.sup", "quali", "quali.sup", "row",
                                        "row.sup", "col", "col.sup"), several.ok = TRUE)
    ...
    if (is.null(palette))
      palette <- c("black", "red", "green3", "blue", "magenta",
                   ...
                   "lightpink", "lavender", "yellow", "darkgreen",
                   "turquoise", "lightgrey", "lightblue", "darkkhaki",
                   "darkmagenta", "lightgreen", "darkolivegreen", "lightcyan",
    ...
    if (graph.type == "ggplot") {
      theme <- theme(axis.title = element_text(hjust = 1,
                                               size = if (is.null(argument[["cex.axis"]])) {
                                                 10
                                               }
                                               else {
                                                 10 * argument$cex.axis
                                               }, face = 2), plot.title = element_text(hjust = 0.5,
                                                                                       size = if (is.null(argument[["cex.main"]])) {
                                                                                         11
                                                                                       }
                                                                                       else {
                                                                                         11 * argument$cex.main
                                                                                       }, face = 2), legend.position = ifelse(legend$x %in%
                                                                                                                                c("bottom", "up", "right", "left"), legend$x, "right"),
    ...
    if (choix == "axes") {
      ...
      for (k in 2:ncp) selectByNCP <- c(selectByNCP, grep(paste("Dim",
                                                                k, sep = ""), rownames(res.mfa$partial.axes$coord)))
      selectByNCP <- sort(selectByNCP)
      res.mfa$partial.axes$coord <- res.mfa$partial.axes$coord[selectByNCP,
                                                               , drop = FALSE]
      res.mfa$partial.axes$contrib <- res.mfa$partial.axes$contrib[selectByNCP,
                                                                   , drop = FALSE]
      res.mfa$partial.axes$cos2 <- res.mfa$partial.axes$cos2[selectByNCP,
                                                             , drop = FALSE]
      ...
      if (habillage == "group") {
        if (is.null(col.hab) | length(col.hab) < length(group)) {
          if (is.null(res.mfa$call$num.group.sup))
            ...
            col.hab[which(!seq_len(length(group)) %in%
                            (res.mfa$call$num.group.sup))] <- 2:(1 +
            ...
            col.hab[res.mfa$call$num.group.sup] <- length(group) -
              ...
              seq_len(length(res.mfa$call$num.group.sup))
          }
        }
        i <- 1
        ...
        couleur.axes <- NULL
        for (i in seq_len(length(group))) couleur.axes <- c(couleur.axes,
      ...
      for (v in seq_len(nrow(coord.axes))) {
        if (graph.type == "classic") {
          arrows(0, 0, coord.axes[v, 1], coord.axes[v,
                                                    2], length = 0.1, angle = 15, code = 2, col = couleur.axes[v],
                 ...)
        }
      ...
      if (graph.type == "classic") {
        ...
        if (autoLab == TRUE)
          autoLab(coord.axes[, 1], y = coord.axes[, 2],
                  labels = labe, col = couleur.axes, shadotext = shadowtext,
                  ...)
      }
      if (habillage == "group") {
      ...
      if (graph.type == "ggplot") {
        circle <- annotate("path", x = 0 + 1 * cos(seq(0,
                                                       2 * pi, length.out = 100)), y = 0 + 1 * sin(seq(0,
                                                                                                       2 * pi, length.out = 100)), lty = ggoptions_default$circle.lty,
                           lwd = ggoptions_default$circle.lwd, color = ggoptions_default$circle.color)
        df_axes <- data.frame(labe, coord.axes)
        group2 <- strsplit(as.character(df_axes[, 1]), ".",
                           fixed = TRUE)
        group <- NULL
        for (i in seq_len(length(group2))) {
          group <- c(group, group2[[i]][2])
        }
        if (habillage == "group") {
          df_axes <- data.frame(df_axes, couleur.axes,
                                group)
          if (!is.null(select))
            df_axes <- df_axes[order(df_axes[, 4]), ]
          gg_graph <- ggplot() + aes(x = df_axes[, 2],
                                     y = df_axes[, 3]) + coord_fixed(ratio = 1) +
            geom_line(aes(x = x, y = y), data = data.frame(x = -1:1,
                                                           y = 0), lty = ggoptions_default$line.lty,
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_line(aes(x = x, y = y), data = data.frame(x = 0,
                                                           y = -1:1), lty = ggoptions_default$line.lty,
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_segment(aes(x = 0, y = 0, xend = df_axes[,
                                                          2], yend = df_axes[, 3], color = as.factor(as.character(df_axes[,
                                                                                                                          4]))), arrow = arrow(length = unit(0.2,
                                                                                                                                                             "cm")), lty = ggoptions_default$segment.lty,
                         lwd = ggoptions_default$segment.lwd) + xlab(lab.x) +
            ylab(lab.y) + ggtitle(title) + labs(color = ifelse(legend["title"] %in%
                                                                 legend, legend["title"][[1]], "")) + theme_light() +
            ggoptions_default$theme
          if (autoLab)
            text <- ggrepel::geom_text_repel(aes(x = df_axes[,
                                                             2], y = df_axes[, 3], label = df_axes[,
                                                                                                   1], color = as.factor(as.character(df_axes[,
                                                                                                                                              4]))), size = ggoptions_default$size, show.legend = FALSE)
          else {
            text <- geom_text(aes(x = df_axes[, 2], y = df_axes[,
                                                                3], label = df_axes[, 1]), color = palette[as.numeric(as.character(df_axes[,
                                                                                                                                           4]))], size = ggoptions_default$size, hjust = (-sign(df_axes[,
                                                                                                                                                                                                        2]) + 1)/2, vjust = -sign(df_axes[, 3]) *
                                0.75 + 0.25)
          }
          if (is.null(select))
            gg_graph <- gg_graph + scale_color_manual(values = palette[L$text.col[order(L$text.col)]],
                                                      labels = unique(df_axes[, 5])[order(unique(df_axes[,
                                                                                                         4]))])
          if (!is.null(select))
            gg_graph <- gg_graph + scale_color_manual(values = palette[unique(df_axes[,
                                                                                      4])], labels = unique(df_axes[, 5]))
        }
        if (habillage == "none") {
          gg_graph <- ggplot() + aes(x = df_axes[, 2],
                                     y = df_axes[, 3]) + coord_fixed(ratio = 1) +
            ...
            geom_line(aes(x = x, y = y), data = data.frame(x = 0,
                      ...
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_segment(aes(x = 0, y = 0, xend = df_axes[,
                                                          2], yend = df_axes[, 3]), arrow = arrow(length = unit(0.2,
                                                                                                                "cm")), lty = ggoptions_default$segment.lty,
                         lwd = ggoptions_default$segment.lwd) + xlab(lab.x) +
            ylab(lab.y) + ggtitle(title) + theme_light() +
            ggoptions_default$theme
          if (autoLab)
            text <- ggrepel::geom_text_repel(aes(x = df_axes[,
                                                             2], y = df_axes[, 3], label = df_axes[,
                                                                                                   1]), size = ggoptions_default$size, show.legend = FALSE)
          else {
            text <- geom_text(aes(x = df_axes[, 2], y = df_axes[,
                                                                3], label = df_axes[, 1]), size = ggoptions_default$size,
                              hjust = (-sign(df_axes[, 2]) + 1)/2, vjust = -sign(df_axes[,
                                                                                         3]) * 0.75 + 0.25)
          }
        }
    ...
    if (choix == "group") {
      ...
      if (habillage == "group" & is.null(col.hab))
        col.hab <- (2:(length(group) + 1))
      if (length(col.hab) == 1)
      ...
      if (!is.null(selection)) {
        if (is.numeric(unselect))
          coll[!(seq_len(length(coll)) %in% selection)] <- rgb(t(col2rgb(coll[!(seq_len(length(coll)) %in%
      ...
      if (!is.null(res.mfa$group$coord.sup)) {
        ...
        if (length(select) == 1) {
          if (grepl("contrib", select)) {
            if (is.numeric(unselect))
              coll2[seq_len(length(coll2))] <- rgb(t(col2rgb(coll2[seq_len(length(coll2))])),
        ...
        if (!is.null(selectionS)) {
          if (is.numeric(unselect))
            coll2[!(seq_len(length(coll2)) %in% selectionS)] <- rgb(t(col2rgb(coll2[!(seq_len(length(coll2)) %in%
      ...
      if (graph.type == "classic") {
        ...
        if (autoLab == "auto")
          autoLab <- (length(labe) < 50)
        if (autoLab == TRUE)
          autoLab(coo[, 1], y = coo[, 2], labels = labe,
                  col = coll, font = fonte, shadotext = shadowtext,
                  ...)
        if (autoLab == FALSE)
          text(coo[, 1], y = coo[, 2], labels = labe,
               col = coll, font = fonte, pos = 3, ...)
        if (!shadowtext)
          points(coo[, 1], y = coo[, 2], pch = ipch, col = coll,
                 ...)
      }
      if (graph.type == "ggplot") {
        ...
        gg_graph <- ggplot() + coord_fixed(ratio = 1) +
          geom_point(aes(x = df_group[, 2], y = df_group[,
                                                         3]), color = rgb(t(col2rgb(df_group[, 4])),
                                                                          maxColorValue = 255), shape = df_group[, 5],
                     size = ggoptions_default$size/3) + xlab(lab.x) +
          ylab(lab.y) + xlim(xlim) + ylim(ylim) + geom_hline(yintercept = 0,
                                                             lty = ggoptions_default$line.lty, lwd = ggoptions_default$line.lwd,
                                                             color = ggoptions_default$line.color) + geom_vline(xintercept = 0,
                                                                                                                lty = ggoptions_default$line.lty, lwd = ggoptions_default$line.lwd,
                                                                                                                color = ggoptions_default$line.color) + theme_light() +
          ggoptions_default$theme + ggtitle(title)
        if (autoLab)
          text <- ggrepel::geom_text_repel(aes(x = df_group[,
                                                            2], y = df_group[, 3], label = df_group[,
                                                                                                    1]), size = ggoptions_default$size, color = rgb(t(col2rgb(df_group[,
                                                                                                                                                                       "coll"])), maxColorValue = 255), fontface = df_group[,
                                                                                                                                                                                                                            "fonte"])
        else {
          text <- geom_text(aes(x = df_group[, 2], y = df_group[,
                                                                3], label = df_group[, 1]), size = ggoptions_default$size,
                            color = rgb(t(col2rgb(df_group[, 4])), maxColorValue = 255),
                            hjust = (-sign(df_group[, 2]) + 1)/2, vjust = -sign(df_group[,
                                                                                         3]) * 0.75 + 0.25, fontface = df_group[,
                                                                                                                                "fonte"])
        }
    ...
    if (choix == "var") {
      ...
      if (habillage == "group") {
        if (is.null(col.hab) | length(col.hab) < length(group[type ==
          ...
          if (!is.null(res.mfa$call$num.group.sup)) {
            ...
            col <- c(1 + rep(which(res.mfa$call$nature.group[-res.mfa$call$num.group.sup] ==
                                     "quanti" | res.mfa$call$nature.group[-res.mfa$call$num.group.sup] ==
                                     "mixed"), times = sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                                                 "quanti" | res.mfa$call$nature.group ==
                                                                                                 "mixed")], function(x) sum("quanti" == x))),
                     rep(length(group) - length(res.mfa$call$num.group.sup) +
                           1 + rep(which(res.mfa$call$nature.group[res.mfa$call$num.group.sup] ==
                                           "quanti.sup" | res.mfa$call$nature.group[res.mfa$call$num.group.sup] ==
                                           "mixed.sup")), times = sapply(res.mfa$call$list.type.var[res.mfa$call$nature.group ==
                                                                                                      "quanti.sup" | res.mfa$call$nature.group ==
                                                                                                      "mixed.sup"], function(x) sum("quanti.sup" ==
                                                                                                                                      x))))
          }
          else {
            col.hab <- 2:(length(group) + 1)
            col <- 1 + rep(which(type == "c" | type ==
                                   "m"), times = sapply(res.mfa$call$list.type.var[res.mfa$call$type ==
                                                                                     "c" | res.mfa$call$type == "m"], function(x) sum("quanti" ==
                                                                                                                                        x)))
          }
        }
      }
        ...
        if (is.null(col.hab) | length(col.hab) < sum(group[type ==
                                                           "c" | type == "m"]))
          col <- rep(1, sum(sapply(res.mfa$call$list.type.var,
                                   function(x) sum("quanti" == x))))
        else col <- col.hab
      }
      if (is.null(title))
      ...
      if (habillage == "group" & is.na(test.invisible[1]) &
        ...
        L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[-nrow(res.mfa$group$Lg),
                                                                    , drop = FALSE])[type == "c" | type == "m"],
                  text.col = col.hab[type == "c" | type == "m"],
                  cex = 0.8 * par("cex"))
        L <- modifyList(L, legend)
        if (graph.type == "classic")
          do.call(graphics::legend, L)
        ...
        if ("quanti.sup" %in% res.mfa$call$nature.var) {
          L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[-c(num.group.sup,
                                                                         nrow(res.mfa$group$Lg)), , drop = FALSE])[type.act ==
                                                                                                                     "c" | type.act == "m"], text.col = col.hab[which(!(seq_len(length(group)) %in%
                                                                                                                                                                          res.mfa$call$num.group.sup))[type.act == "c" |
                                                                                                                                                                                                         type.act == "m"]], cex = 0.8 * par("cex"))
        }
        else {
          L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[-nrow(res.mfa$group$Lg),
          ])[type == "c" | type == "m"], text.col = col.hab[type ==
                                                              "c" | type == "m"], cex = 0.8 * par("cex"))
        }
        L <- modifyList(L, legend)
        if (graph.type == "classic")
          do.call(graphics::legend, L)
      ...
      if (habillage == "group" & !is.na(test.invisible[1]) &
          is.na(test.invisible[2])) {
        if (any(c("quanti", "mixed") %in% res.mfa$call$nature.var))
          L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[num.group.sup,
                                                                      , drop = FALSE])[which(type.sup == "c" | type.sup ==
                                                                                               "m")], text.col = col.hab[res.mfa$call$num.group.sup[which(type.sup ==
                                                                                                                                                            "c" | type.sup == "m")]], cex = 0.8 * par("cex"))
        else L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[num.group.sup,
                                                                         , drop = FALSE])[type.sup == "c" | type == "m"],
                       text.col = col.hab[res.mfa$call$num.group.sup[type.sup ==
                                                                       "c" | type.sup == "m"]], cex = 0.8 * par("cex"))
        L <- modifyList(L, legend)
        if (graph.type == "classic")
          do.call(graphics::legend, L)
      ...
      if ((!is.null(res.mfa["quanti.var"]$quanti.var)) & (!is.na(test.invisible[1]))) {
        col[seq_len(nrow(res.mfa$quanti.var$cor[, axes,
                                                drop = FALSE]))] <- "transparent"
      }
      if (!is.null(res.mfa["quanti.var"]$quanti.var)) {
        ...
        if (graph.type == "classic") {
          for (v in seq_len(nrow(coord.var))) {
            if (!(is.nan(coord.var[v, 1]) | is.infinite(coord.var[v,
                                                                  1]))) {
              arrows(0, 0, coord.var[v, 1], coord.var[v,
                                                      2], length = 0.1, angle = 15, code = 2,
                     col = col[v])
              if (lab.var) {
      ...
      if ((!is.null(res.mfa$quanti.var.sup$coord)) & (!is.na(test.invisible[2]))) {
        col[nrow(coo) + seq_len(nrow(res.mfa$quanti.var.sup$cor[,
      ...
      if (!is.null(res.mfa$quanti.var.sup$coord)) {
        ...
        if (graph.type == "classic") {
          for (q in seq_len(nrow(coord.quanti))) {
            if (!(is.nan(coord.quanti[q, 1]) | is.infinite(coord.quanti[q,
                                                                        1]))) {
              arrows(0, 0, coord.quanti[q, 1], coord.quanti[q,
                                                            2], length = 0.1, angle = 15, code = 2,
                     lty = 2, col = col[nrow(coo) - nrow(coord.quanti) +
                                          q], ...)
              if (lab.var) {
      ...
      if (graph.type == "classic") {
        if (autoLab == "auto")
          autoLab <- (length(labe) - sum(col == "transparent") <
                        50)
        AGarder <- which(!(is.nan(coo[, 1]) | is.infinite(coo[,
                                                              1])))
        coo <- coo[AGarder, ]
        col <- col[AGarder]
        labe <- labe[AGarder]
        if (autoLab == FALSE)
          text(coo[, 1], y = coo[, 2], labels = labe,
               pos = posi, col = col, ...)
        if (autoLab == TRUE)
          autoLab(coo[which(col != "transparent"), 1],
                  y = coo[which(col != "transparent"), 2], labels = labe[which(col !=
                                                                                 "transparent")], col = col[which(col !=
                                                                                                                    "transparent")], shadotext = shadowtext,
                  ...)
      ...
      if (graph.type == "ggplot") {
        ...
        transparency_var <- 1
        if (!is.null(select))
          transparency_var <- ifelse(df_var[, 1] != "",
                                     1, 1 - unselect)
        if (habillage == "group") {
          ...
          df_var <- df_var[which(!(is.nan(df_var[, 2]) |
                                     is.infinite(df_var[, 2]))), ]
          gg_graph <- ggplot() + coord_fixed(ratio = 1) +
            geom_line(aes(x = x, y = y), data = data.frame(x = -1:1,
                                                           y = 0), lty = ggoptions_default$line.lty,
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_line(aes(x = x, y = y), data = data.frame(x = 0,
                                                           y = -1:1), lty = ggoptions_default$line.lty,
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_segment(aes(x = 0, y = 0, xend = df_var[,
                                                         2], yend = df_var[, 3], color = df_var[,
                                                                                                4]), arrow = arrow(length = unit(0.2, "cm")),
                         lty = ggoptions_default$segment.lty, lwd = ggoptions_default$segment.lwd,
                         alpha = df_var[, 5]) + xlab(lab.x) + ylab(lab.y) +
            xlim(c(-1.3, 1.3)) + ylim(c(-1.1, 1.1)) +
            ggtitle(title) + labs(color = ifelse(legend["title"] %in%
                                                   legend, legend["title"][[1]], "")) + scale_color_manual(values = palette[L$text.col[order(L$text.col)]],
                                                                                                           labels = L$legend[order(L$text.col)]) + theme_light() +
            ggoptions_default$theme
          if (autoLab)
            text <- ggrepel::geom_text_repel(aes(x = df_var[,
                                                            2], y = df_var[, 3], label = df_var[, 1],
                                                 color = df_var[, 4]), size = ggoptions_default$size,
                                             show.legend = FALSE)
          else {
            text <- geom_text(aes(x = df_var[, 2], y = df_var[,
                                                              3], label = df_var[, 1], color = df_var[,
                                                                                                      4]), size = ggoptions_default$size, hjust = (-sign(df_var[,
                                                                                                                                                                2]) + 1)/2, vjust = -sign(df_var[, 3]) *
                                0.75 + 0.25, show.legend = FALSE)
          }
        }
        if (habillage == "none") {
          df_var <- data.frame(df_var, col, transparency_var)
          ...
          gg_graph <- ggplot() + coord_fixed(ratio = 1) +
            ...
            geom_line(aes(x = x, y = y), data = data.frame(x = 0,
                      ...
                      lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
            geom_segment(aes(x = 0, y = 0, xend = df_var[,
                                                         2], yend = df_var[, 3]), arrow = arrow(length = unit(0.2,
                                                                                                              "cm")), lty = ggoptions_default$segment.lty,
                         lwd = ggoptions_default$segment.lwd, alpha = df_var[,
                                                                             5]) + xlab(lab.x) + ylab(lab.y) + ggtitle(title) +
            theme_light() + ggoptions_default$theme
          if (autoLab)
            text <- ggrepel::geom_text_repel(aes(x = df_var[,
                                                            2], y = df_var[, 3], label = df_var[, 1]),
                                             size = ggoptions_default$size, show.legend = FALSE)
          else {
            text <- geom_text(aes(x = df_var[, 2], y = df_var[,
                                                              3], label = df_var[, 1]), size = ggoptions_default$size,
                              hjust = (-sign(df_var[, 2]) + 1)/2, vjust = -sign(df_var[,
                                                                                       3]) * 0.75 + 0.25, show.legend = FALSE)
          }
        }
        gg_graph <- gg_graph + text + theme + circle
      }
    ...
    if (choix == "freq") {
      if (is.null(res.mfa$freq) & is.null(res.mfa$freq.sup))
        stop("You have no frequencies groups")
      if ((new.plot) & !nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY")))
        dev.new()
      col.row <- 1
      col.row.sup <- "grey60"
        ...
        if (is.null(col.hab) | length(col.hab) < length(group[type ==
          ...
          if (!is.null(res.mfa$call$num.group.sup)) {
            ...
            col.hab[res.mfa$call$num.group.sup] <- length(group) -
              length(res.mfa$call$num.group.sup) + 1 +
              seq_len(length(res.mfa$call$num.group.sup))
            if (!is.null(res.mfa$call$nature.group ==
                         "contingency"))
              col.col <- rep(col.hab[res.mfa$call$nature.group ==
                                       "contingency"], times = group[res.mfa$call$nature.group ==
                                                                       "contingency"])
            if (!is.null(res.mfa$call$nature.group ==
                         "contingency.sup"))
              col.col.sup <- rep(col.hab[res.mfa$call$nature.group ==
                                           "contingency.sup"], times = group[res.mfa$call$nature.group ==
                                                                               "contingency.sup"])
          }
          else {
            col.hab <- 2:(length(group) + 1)
            col.col <- 1 + rep(which(type == "f"), times = group[type ==
                                                                   "f"])
          }
        }
      }
      ...
      if (is.na(test.invisible[2])) {
        ...
        if (!is.null(selectionC)) {
          if (is.numeric(unselect))
            coll2[!(seq_len(length(coll2)) %in% selectionC)] <- rgb(t(col2rgb(coll2[!(seq_len(length(coll2)) %in%
      ...
      if (!is.null(res.mfa$freq.sup) & is.na(test.invisible[4])) {
        ...
        if (!is.null(selectionCS)) {
          if (is.numeric(unselect))
            coll2[!(seq_len(length(coll2)) %in% selectionCS)] <- rgb(t(col2rgb(coll2[!(seq_len(length(coll2)) %in%
      ...
      if (habillage == "group") {
        if (is.na(test.invisible[1]))
          L <- list(x = "topleft", legend = c("Individuals",
                                              rownames(res.mfa$group$Lg[-nrow(res.mfa$group$Lg),
                                                                        , drop = FALSE])[type == "f"]), text.col = c(1,
                                                                                                                     col.hab[type == "f"]), cex = 0.8 * par("cex"))
        else L <- list(x = "topleft", legend = rownames(res.mfa$group$Lg[-nrow(res.mfa$group$Lg),
                                                                         , drop = FALSE])[type == "f"], text.col = col.hab[type ==
                                                                                                                             "f"], cex = 0.8 * par("cex"))
        L <- modifyList(L, legend)
        if (graph.type == "classic")
          do.call(graphics::legend, L)
      ...
      if (graph.type == "ggplot") {
        ...
        if (habillage == "group") {
          gg_graph <- ggplot() + coord_fixed(ratio = 1) +
            geom_point(aes(x = df_freq[, 2], y = df_freq[,
                                                         3]), color = palette[df_freq[, 4]], shape = ggoptions_default$point.shape,
                       size = ggoptions_default$size/3) + xlab(lab.x) +
            ylab(lab.y) + xlim(xlim) + ylim(ylim) + geom_hline(yintercept = 0,
                                                               lty = ggoptions_default$line.lty, lwd = ggoptions_default$line.lwd,
                                                               color = ggoptions_default$line.color) + geom_vline(xintercept = 0,
                                                                                                                  lty = ggoptions_default$line.lty, lwd = ggoptions_default$line.lwd,
                                                                                                                  color = ggoptions_default$line.color) + labs(color = ifelse(legend["title"] %in%
                                                                                                                                                                                legend, legend["title"][[1]], "")) + scale_color_manual(values = palette[L$text.col[order(L$text.col)]],
                                                                                                                                                                                                                                        labels = L$legend[order(L$text.col)]) + theme_light() +
            ggoptions_default$theme + ggtitle(titre)
          if (autoLab)
            text <- ggrepel::geom_text_repel(aes(x = df_freq[,
                                                             2], y = df_freq[, 3], label = df_freq[,
                                                                                                   1]), size = ggoptions_default$size, color = palette[df_freq[,
                                                                                                                                                               4]])
          else {
            text <- geom_text(aes(x = df_freq[, 2], y = df_freq[,
                                                                3], label = df_freq[, 1]), size = ggoptions_default$size,
                              color = palette[df_freq[, 4]], hjust = (-sign(df_freq[,
                                                                                    2]) + 1)/2, vjust = -sign(df_freq[, 3]) *
                                0.75 + 0.25)
          }
        }
        if (habillage == "none") {
    ...
    if (choix == "ind") {
      ...
      if (!is.null(res.mfa["quali.var"]$quali.var)) {
        ...
        coord_quali.partiel <- res.mfa$quali.var$coord.partiel[,
                                                               axes, drop = FALSE]
      }
      if (!is.null(res.mfa["quali.var.sup"])) {
        coord_quali_sup <- res.mfa$quali.var.sup$coord[,
                                                       axes, drop = FALSE]
        coord_quali.partiel.sup <- res.mfa$quali.var.sup$coord.partiel[,
                                                                       axes, drop = FALSE]
      }
      ...
      if (!is.null(partial)) {
        if (length(partial) == 1) {
          if (partial == "all") {
            ...
            if (!is.null(res.mfa$ind.sup))
              group.ind.sup <- seq_len(nrow(coord.ind.sup))
            if (!is.null(res.mfa["quali.var"]$quali.var))
              group.quali <- seq_len(nrow(coord_quali))
            if (!is.null(res.mfa["quali.var.sup"]$quali.var.sup))
              group.quali.sup <- seq_len(nrow(coord_quali_sup))
          }
          else {
            for (i in seq_len(length(partial))) {
              ...
              if (partial[i] %in% rownames(coord_quali_sup))
                group.quali.sup <- c(group.quali.sup,
                                     match(partial[i], rownames(coord_quali_sup)))
            }
          }
        }
        else {
      ...
      if (is.null(xlim)) {
        ...
        if (!is.null(res.mfa["quali.var"]$quali.var) & is.na(test.invisible[3]))
          xmin <- min(xmin, coord_quali.partiel[unlist(lapply(group.quali,
                                                              function(k) seq(nbre.grpe * (k - 1) + 1, length = nbre.grpe))),
                                                1])
        if (!is.null(res.mfa["quali.var"]$quali.var) & is.na(test.invisible[3]))
        ...
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
          xmin <- min(xmin, coord_quali[, 1], coord_quali_sup[,
                                                              1])
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
          xmax <- max(xmax, coord_quali[, 1], coord_quali_sup[,
                                                              1])
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
      ...
      if (is.null(ylim)) {
        ...
        if (!is.null(res.mfa["quali.var"]$quali.var) & is.na(test.invisible[3]))
          ymax <- max(ymax, coord_quali.partiel[unlist(lapply(group.quali,
                                                ...
                                                2])
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
          ymin <- min(ymin, coord_quali[, 1], coord_quali_sup[,
                                                              2])
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
          ymax <- max(ymax, coord_quali[, 1], coord_quali_sup[,
                                                              2])
        if (!is.null(res.mfa$quali.var.sup) & is.na(test.invisible[4]))
      ...
      if (nullxlimylim & diff(xlim)/diff(ylim) > 3)
        ylim <- (ylim - mean(ylim)) * diff(xlim)/diff(ylim)/3 +
        mean(ylim)
      if (nullxlimylim & diff(xlim)/diff(ylim) < 1/2)
        xlim <- (xlim - mean(xlim)) * diff(ylim)/diff(xlim)/2 +
        mean(xlim)
      if (graph.type == "ggplot")
      ...
      if ((!is.null(select)) & (!is.null(res.mfa$ind.sup$coord)) &
        ...
        if (mode(select) == "numeric")
          ...
          if (sum(rownames(res.mfa$ind$coord) %in% select) +
              sum(rownames(res.mfa$ind.sup$coord) %in% select) !=
              0)
            selectionS <- which(rownames(res.mfa$ind.sup$coord) %in%
                                  select)
          else {
            if (grepl("dist", select[1]))
              selectionS <- (rev(order(res.mfa$ind.sup$dist)))[1:min(nrow(res.mfa$ind.sup$coord),
                                                                     sum(as.integer(unlist(strsplit(select,
                                                                                                    "dist"))), na.rm = T))]
            if (grepl("coord", select[1]))
              selectionS <- (rev(order(apply(res.mfa$ind.sup$coord[,
                                                                   axes]^2, 1, sum))))[1:min(nrow(res.mfa$ind.sup$coord),
                                                                                             sum(as.integer(unlist(strsplit(select,
                                                                                                                            "coord"))), na.rm = T))]
            if (grepl("cos2", select[1])) {
              if (sum(as.numeric(unlist(strsplit(select,
                                                 "cos2"))), na.rm = T) >= 1)
                selectionS <- (rev(order(apply(res.mfa$ind.sup$cos2[,
                                                                    axes, drop = FALSE], 1, sum))))[1:min(nrow(res.mfa$ind.sup$coord),
                                                                                                          sum(as.numeric(unlist(strsplit(select,
                                                                                                                                         "cos2"))), na.rm = T))]
              else selectionS <- which(apply(res.mfa$ind.sup$cos2[,
      ...
      if (habillage == "none") {
        ...
        if (!is.null(res.mfa$ind.sup))
          col.ind.sup <- rep(4, (nb.ind - nb.ind.actif) *
                               (nbre.grpe + 1))
        if (!is.null(res.mfa[["quali.var"]]))
          col.quali <- rep(2, (nbre.grpe + 1) * sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                                   "quali" | res.mfa$call$nature.group == "mixed")]) -
                             sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                           "quali" | res.mfa$call$nature.group == "mixed")],
                                        function(x) sum(x %in% "quanti"))))
        if (!is.null(res.mfa$quali.var.sup))
          col.quali.sup <- rep("darkred", (nbre.grpe +
                                             1) * sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
        ...
        if (!is.null(ellipse))
          col.ellipse <- rep(25, nb.ind.actif)
        if (!is.null(ellipse.par))
          col.ellipse.par <- rep(2, nb.ind.actif * (nbre.grpe +
                                                      1))
      }
      if (habillage == "group") {
        ...
        if (!is.null(res.mfa[["quali.var"]]))
          col.quali <- c(rep(1, sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                   "quali" | res.mfa$call$nature.group == "mixed")]) -
                               sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                             "quali" | res.mfa$call$nature.group == "mixed")],
                                          function(x) sum(x %in% "quanti")))), rep(col.hab,
                                                                                   sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                                                                      "quali" | res.mfa$call$nature.group == "mixed")]) -
                                                                                     sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                                                                                   "quali" | res.mfa$call$nature.group ==
                                                                                                                                   "mixed")], function(x) sum(x %in% "quanti")))))
        if (!is.null(res.mfa$quali.var.sup))
          col.quali.sup <- c(rep(1, sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                       "quali.sup" | res.mfa$call$nature.group ==
                                                                       "mixed.sup")]) - sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                                                                                      "quali.sup" | res.mfa$call$nature.group ==
                                                                                                                                      "mixed.sup")], function(x) sum(x %in% "quanti.sup")))),
                             rep(col.hab, sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                             "quali.sup" | res.mfa$call$nature.group ==
                                                                             "mixed.sup")]) - sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                                                                                            "quali.sup" | res.mfa$call$nature.group ==
                                                                                                                                            "mixed.sup")], function(x) sum(x %in% "quanti.sup")))))
      ...
      if (habillage == "ind") {
        ...
        if (!is.null(res.mfa[["quali.var"]]))
          col.quali <- rep("black", (1 + nbre.grpe) *
                             sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                "quali" | res.mfa$call$nature.group == "mixed")]) -
                             sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                           "quali" | res.mfa$call$nature.group == "mixed")],
                                        function(x) sum(x %in% "quanti"))))
        if (!is.null(res.mfa$quali.var.sup))
          col.quali.sup <- rep("black", (1 + nbre.grpe) *
                                 sum(res.mfa$call$group.mod[which(res.mfa$call$nature.group ==
                                                                    "quali.sup" | res.mfa$call$nature.group ==
                                                                    "mixed.sup")]) - sum(sapply(res.mfa$call$list.type.var[which(res.mfa$call$nature.group ==
                                                                                                                                   "quali.sup" | res.mfa$call$nature.group ==
      ...
      if ((habillage != "none") & (habillage != "ind") & (habillage !=
        ...
        for (i in group.act) {
          if (type[i] == "n") {
            for (k in seq_len(ncol(res.mfa$separate.analyses[[i]]$call$X))) nbre.modalite <- c(nbre.modalite,
          ...
          if (type[i] == "m") {
            for (k in which(!sapply(res.mfa$separate.analyses[[i]]$call$X,
                                    is.numeric))) nbre.modalite <- c(nbre.modalite,
                                                                     nlevels(res.mfa$separate.analyses[[i]]$call$X[,
                                                                                                                   k]))
            liste.quali <- c(liste.quali, colnames(res.mfa$call$X)[which(!sapply(res.mfa$separate.analyses[[i]]$call$X,
                                                                                 is.numeric))])
          }
        }
        if (!is.null(num.group.sup)) {
                ...
                liste.quali.sup <- c(liste.quali.sup,
                                     colnames(res.mfa$call$X[1:group[1]]))
                                        ...
                                        colnames(res.mfa$call$X[(sum(group[1:(i -
                                                                                1)]) + 1):sum(group[1:i])]))
              for (k in seq_len(ncol(res.mfa$separate.analyses[[i]]$call$X))) nbre.modalite.sup <- c(nbre.modalite.sup,
                                                                                                     nlevels(res.mfa$separate.analyses[[i]]$call$X[,
                                                                                                                                                   k]))
            }
            if (type[i] == "m") {
              for (k in which(!sapply(res.mfa$separate.analyses[[i]]$call$X,
                                      is.numeric))) nbre.modalite.sup <- c(nbre.modalite.sup,
                                                                           nlevels(res.mfa$separate.analyses[[i]]$call$X[,
                                                                                                                         k]))
              liste.quali.sup <- c(liste.quali.sup, colnames(res.mfa$call$X)[which(!sapply(res.mfa$separate.analyses[[i]]$call$X,
                                                                                           is.numeric))])
            }
          }
        }
        ...
        col.quali <- rep("black", sum(res.mfa$call$group.mod[type ==
                                                               "n"]))
        if (sum(type == "m") > 0)
          col.quali <- c(col.quali, rep("black", sum(res.mfa$call$group.mod[type ==
                                                                              "m"]) - sum(sapply(res.mfa$call$list.type.var[type ==
                                                                                                                              "m"], function(x) sum("quanti" == x))) - sum(sapply(res.mfa$call$list.type.var[type ==
                                                                                                                                                                                                               "m"], function(x) sum("quanti.sup" == x)))))
        if (nom.quali %in% liste.quali) {
          indice.inf <- sum(nbre.modalite[0:(match(nom.quali,
                                                   liste.quali) - 1)]) + 1
          indice.sup <- indice.inf + length(modalite) -
            1
          if (length(group[type == "n"]) + length(group[type ==
                                                        "m"]) > 0) {
        ...
        col.quali.sup <- rep("black", sum(res.mfa$call$group.mod[(type ==
                                                                    "n") %in% num.group.sup]))
        if (sum((type == "m") %in% num.group.sup) > 0)
          col.quali.sup <- c(col.quali.sup, rep("black",
                                                sum(res.mfa$call$group.mod[(type == "m") %in%
                                                                             num.group.sup]) - sum(sapply(res.mfa$call$list.type.var[(type ==
                                                                                                                                        "m") %in% num.group.sup], function(x) sum("quanti.sup" ==
                                                                                                                                                                                    x)))))
      ...
      if ((new.plot) & !nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY")))
        dev.new(width = min(14, max(8, 8 * diff(xlim)/diff(ylim))),
                height = 8)
      if (is.null(title))
      ...
      if (graph.type == "ggplot") {
        gg_graph <- ggplot() + coord_fixed(ratio = 1) +
          xlab(lab.x) + ylab(lab.y) + xlim(xlim) + ylim(ylim) +
          geom_hline(yintercept = 0, lty = ggoptions_default$line.lty,
                     lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
          geom_vline(xintercept = 0, lty = ggoptions_default$line.lty,
                     lwd = ggoptions_default$line.lwd, color = ggoptions_default$line.color) +
          theme_light() + ggoptions_default$theme + ggtitle(title)
      }
      if (is.na(test.invisible[1])) {
        ...
        if (!is.null(selection)) {
          ...
          coll[(seq_len(length(coll)) %in% selection)] <- rgb(t(col2rgb(coll[(seq_len(length(coll)) %in%
                                                                                selection)])), maxColorValue = 255)
          labe[!(seq_len(length(coll)) %in% selection)] <- ""
        }
        transparency_ind <- col2rgb(coll, alpha = TRUE)[4,
        ]/255
        coll_bis <- rgb(t(col2rgb(coll)), maxColorValue = 255)
        if (graph.type == "ggplot") {
          df_ind <- cbind.data.frame(labe, coord.ind,
                                     coll, ipch, fonte, coll_bis)
          df_ind[, 5][which(df_ind[, 5] == 20)] <- 19
          if (habillage %in% c("none", "ind", "group")) {
            gg_graph <- gg_graph + geom_point(aes(x = df_ind[,
                                                             2], y = df_ind[, 3]), colour = df_ind[,
                                                                                                   7], alpha = transparency_ind, shape = df_ind[,
                                                                                                                                                5], size = ggoptions_default$size/3)
            if (autoLab)
              text_ind <- ggrepel::geom_text_repel(aes(x = df_ind[,
                                                                  2], y = df_ind[, 3], label = df_ind[,
                                                                                                      1]), size = ggoptions_default$size, colour = df_ind[,
                                                                                                                                                          7], fontface = df_ind[, 6])
            else {
              text_ind <- geom_text(aes(x = df_ind[, 2],
                                        y = df_ind[, 3], label = df_ind[, 1]),
                                    size = ggoptions_default$size, colour = df_ind[,
                                                                                   7], hjust = (-sign(df_ind[, 2]) + 1)/2,
                                    vjust = -sign(df_ind[, 3]) * 0.75 + 0.25,
                                    fontface = df_ind[, 6])
            }
          }
          else {
            transparency_ind <- ifelse(rownames(df_ind) %in%
                                         df_ind[, 1], 1, 1 - unselect)
            if (class(habillage) %in% c("numeric", "integer"))
              habillage <- colnames(res.mfa$call$X)[habillage]
            if (habillage %in% colnames(res.mfa$call$X)) {
              gg_graph <- gg_graph + geom_point(aes(x = df_ind[,
                                                               2], y = df_ind[, 3], color = res.mfa$call$X[rownames(df_ind),
                                                                                                           habillage]), shape = ggoptions_default$point.shape,
                                                size = ggoptions_default$size/3, alpha = transparency_ind) +
                scale_color_manual(values = palette[sort(unique(coll_num))]) +
                labs(color = ifelse(legend["title"] %in%
                                      legend, legend["title"][[1]], habillage))
              if (autoLab)
                text_ind <- ggrepel::geom_text_repel(aes(x = df_ind[,
                                                                    2], y = df_ind[, 3], label = df_ind[,
                                                                                                        1], color = (res.mfa$call$X)[rownames(df_ind),
                                                                                                                                     habillage]), size = ggoptions_default$size,
                                                     show.legend = FALSE)
              else {
                text_ind <- geom_text(aes(x = df_ind[,
                                                     2], y = df_ind[, 3], label = df_ind[,
                                                                                         1], color = res.mfa$call$X[rownames(df_ind),
                                                                                                                    habillage]), size = ggoptions_default$size,
                                      show.legend = FALSE, hjust = (-sign(df_ind[,
                                                                                 2]) + 1)/2, vjust = -sign(df_ind[,
                                                                                                                  3]) * 0.75 + 0.25)
              }
            }
          }
        ...
        if (!is.null(partial)) {
          if (graph.type == "classic") {
            for (i in group.ind.actif) {
              if (col2rgb(coll[i], alpha = TRUE)[4] ==
                  255) {
                for (j in 1:nbre.grpe) {
                  points(coord.ind.partiel[(i - 1) * nbre.grpe +
                                             j, 1], coord.ind.partiel[(i - 1) *
                                                                        nbre.grpe + j, 2], cex = 0.8 * par("cex"),
                         col = col.ind[nb.ind.actif + (i -
                                                         1) * nbre.grpe + j], pch = 20)
                  if (lab.par)
                    text(coord.ind.partiel[(i - 1) * nbre.grpe +
                                             j, 1], y = coord.ind.partiel[(i -
                                                                             1) * nbre.grpe + j, 2], labels = rownames(coord.ind.partiel)[(i -
            ...
            liste_ind_partiel <- rep(nbre.grpe * (group.ind.actif -
                                                    1), each = nbre.grpe) + rep(1:nbre.grpe,
                                                                                length(group.ind.actif))
            df_ind_partial <- cbind.data.frame(matrix(rep(cbind(coord.ind[group.ind.actif,
                                                                          , drop = FALSE], col2rgb(coll[group.ind.actif],
                                                                                                   alpha = TRUE)[4, ]), each = nbre.grpe),
                                                      ncol = 3), x = coord.ind.partiel[liste_ind_partiel,
                                                                                       1], y = coord.ind.partiel[liste_ind_partiel,
                                                                                                                 2], coul = col.ind[nb.ind.actif + (1:(length(group.ind.actif) *
                                                                                                                                                         nbre.grpe))], group = rep(1:nbre.grpe, length(group.ind.actif)))
            colnames(df_ind_partial)[1:3] <- c("xfin",
                                               "yfin", "sel")
            df_ind_partial <- df_ind_partial[df_ind_partial[,
                                                            "sel"] == 255, ]
            text <- NULL
            if (lab.par) {
              if (autoLab)
                text <- ggrepel::geom_text_repel(data = df_ind_partial,
                                                 aes(x = x, y = y, label = rownames(df_ind_partial),
                                                     color = as.factor(coul)))
              else text <- geom_text(aes_string(data = df_ind_partial,
                                                x = x, y = y, label = rownames(df_ind_partial),
                                                color = coul))
            }
            if (habillage == "group")
              gg_graph <- gg_graph + geom_point(data = df_ind_partial,
                                                aes(x = x, y = y, color = as.factor(coul)),
                                                shape = 19, size = ggoptions_default$size/3) +
              text
            else gg_graph <- gg_graph + geom_point(data = df_ind_partial,
                                                   aes(x = x, y = y), col = palette[df_ind_partial[,
                                                                                                   "coul"]], shape = 19, size = ggoptions_default$size/3) +
              text
            if (!chrono) {
              if (habillage == "group")
                gg_graph <- gg_graph + geom_segment(aes(x = x,
                                                        y = y, xend = xfin, yend = yfin, col = as.factor(coul)),
                                                    lty = df_ind_partial$coul, data = df_ind_partial)
              else gg_graph <- gg_graph + geom_segment(data = df_ind_partial,
                                                       aes(x = x, y = y, xend = xfin, yend = yfin),
                                                       col = palette[df_ind_partial[, "coul"]],
                                                       lty = df_ind_partial$group)
            }
            else {
              for (i in group.ind.actif) {
                if (col2rgb(coll[i], alpha = TRUE)[4] ==
                  ...
                  for (j in 2:nbre.grpe) {
                    gg_graph <- gg_graph + geom_line(aes_string(x = c(coord.ind.partiel[(i -
                                                                                           1) * nbre.grpe + (j - 1), 1], coord.ind.partiel[(i -
                                                                                                                                              1) * nbre.grpe + j, 1]), y = c(coord.ind.partiel[(i -
                                                                                                                                                                                                  1) * nbre.grpe + (j - 1), 2], coord.ind.partiel[(i -
                                                                                                                                                                                                                                                     1) * nbre.grpe + j, 2])), color = col.ind[i])
                  }
                }
              }
            }
      ...
      if (!is.null(res.mfa$ind.sup) & is.na(test.invisible[2])) {
        ...
        if (graph.type == "ggplot") {
          transparency_ind <- col2rgb(coll2, alpha = TRUE)[4,
          ]/255
          coll_bis <- rgb(t(col2rgb(coll2)), maxColorValue = 255)
          df_ind.sup <- data.frame(labe2, coord.ind.sup,
                                   coll2, rep(21, nrow(coord.ind.sup)), rep(3,
                                                                            nrow(coord.ind.sup)), coll_bis)
          gg_graph <- gg_graph + geom_point(aes(x = df_ind.sup[,
                                                               2], y = df_ind.sup[, 3]), color = df_ind.sup[,
                                                                                                            7], alpha = transparency_ind, shape = df_ind.sup[,
                                                                                                                                                             5], size = ggoptions_default$size/3)
          if (autoLab)
            text_ind.sup <- ggrepel::geom_text_repel(aes(x = df_ind.sup[,
                                                                        2], y = df_ind.sup[, 3], label = df_ind.sup[,
                                                                                                                    1]), size = ggoptions_default$size, color = df_ind.sup[,
                                                                                                                                                                           7], fontface = df_ind.sup[, 6])
          else {
            text_ind.sup <- geom_text(aes(x = df_ind.sup[,
                                                         2], y = df_ind.sup[, 3], label = df_ind.sup[,
                                                                                                     1]), size = ggoptions_default$size, color = df_ind.sup[,
                                                                                                                                                            7], hjust = (-sign(df_ind.sup[, 2]) + 1)/2,
                                      vjust = -sign(df_ind.sup[, 3]) * 0.75 +
                                        0.25, fontface = df_ind.sup[, 6])
          }
        }
        if (!is.null(partial)) {
          if (graph.type == "classic") {
            for (i in group.ind.sup) {
              for (j in 1:nbre.grpe) {
                points(coord.ind.partiel.sup[(i - 1) *
                                               nbre.grpe + j, 1], coord.ind.partiel.sup[(i -
                                                                                           1) * nbre.grpe + j, 2], cex = 0.8 *
                         par("cex"), col = col.ind.sup[nb.ind -
                                                         nb.ind.actif + (i - 1) * nbre.grpe +
                                                         j], pch = 21)
                if (lab.par)
                  text(coord.ind.partiel.sup[(i - 1) *
                                               nbre.grpe + j, 1], y = coord.ind.partiel.sup[nb.ind +
                ...
                if (chrono) {
                  if (j > 1) {
                    if (graph.type == "classic")
                      lines(c(coord.ind.partiel.sup[(i -
                                                       1) * nbre.grpe + (j - 1), 1],
                              coord.ind.partiel.sup[(i - 1) *
                                                      nbre.grpe + j, 1]), c(coord.ind.partiel.sup[(i -
                                                                                                     1) * nbre.grpe + (j - 1), 2],
                                                                            coord.ind.partiel.sup[(i - 1) *
                                                                                                    nbre.grpe + j, 2]), col = col.ind[nb.ind.actif +
                                                                                                                                        i])
                  }
                }
                else lines(c(coord.ind.sup[i, 1], coord.ind.partiel.sup[(i -
                                                                           1) * nbre.grpe + j, 1]), c(coord.ind.sup[i,
                                                                                                                    2], coord.ind.partiel.sup[(i - 1) *
                                                                                                                                                nbre.grpe + j, 2]), col = col.ind.sup[nb.ind -
                                                                                                                                                                                        nb.ind.actif + (i - 1) * nbre.grpe +
                                                                                                                                                                                        j], lty = j)
              }
            }
            ...
            liste_ind_sup_partiel <- rep(nbre.grpe * (group.ind.sup -
                                                        1), each = nbre.grpe) + rep(1:nbre.grpe,
                                                                                    length(group.ind.sup))
            df_ind_partial_sup <- cbind.data.frame(matrix(rep(cbind(coord.ind.sup[group.ind.sup,
                                                                                  , drop = FALSE], col2rgb(coll2[group.ind.sup],
                                                                                                           alpha = TRUE)[4, ]), each = nbre.grpe),
                                                          ncol = 3), x = coord.ind.partiel.sup[liste_ind_sup_partiel,
                                                                                               1], y = coord.ind.partiel.sup[liste_ind_sup_partiel,
                                                                                                                             2], coul = col.ind.sup[(nb.ind - nb.ind.actif) +
                                                                                                                                                      liste_ind_sup_partiel], group = rep(1:nbre.grpe,
                                                                                                                                                                                          length(group.ind.sup)))
            colnames(df_ind_partial_sup)[1:3] <- c("xfin",
                                                   "yfin", "sel")
            df_ind_partial_sup <- df_ind_partial_sup[df_ind_partial_sup[,
                                                                        "sel"] == 255, ]
            if (habillage == "group")
              gg_graph <- gg_graph + geom_point(data = df_ind_partial_sup,
                                                aes(x = x, y = y, color = as.factor(coul)),
                                                shape = 21, size = ggoptions_default$size/3)
            else gg_graph <- gg_graph + geom_point(data = df_ind_partial_sup,
                                                   aes(x = x, y = y), color = palette[df_ind_partial_sup[,
                                                                                                         "coul"]], shape = 21, size = ggoptions_default$size/3)
            if (!chrono) {
              if (habillage == "group")
                gg_graph <- gg_graph + geom_segment(aes(x = x,
                                                        y = y, xend = xfin, yend = yfin, col = as.factor(coul)),
                                                    lty = df_ind_partial_sup$coul, data = df_ind_partial_sup)
              else gg_graph <- gg_graph + geom_segment(aes(x = x,
                                                           y = y, xend = xfin, yend = yfin), col = palette[df_ind_partial_sup[,
                                                                                                                              "coul"]], lty = df_ind_partial_sup$group,
                                                       data = df_ind_partial_sup)
            }
            else {
              for (i in group.ind.sup) {
                if (col2rgb(coll[i], alpha = TRUE)[4] ==
                    255) {
                  for (j in 2:nbre.grpe) {
                    gg_graph <- gg_graph + geom_line(aes_string(x = c(coord.ind.partiel.sup[(i -
                                                                                               1) * nbre.grpe + (j - 1), 1], coord.ind.partiel.sup[(i -
                                                                                                                                                      1) * nbre.grpe + j, 1]), y = c(coord.ind.partiel.sup[(i -
                                                                                                                                                                                                              1) * nbre.grpe + (j - 1), 2], coord.ind.partiel.sup[(i -
                                                                                                                                                                                                                                                                     1) * nbre.grpe + j, 2])), color = col.ind[nb.ind.actif +
                                                                                                                                                                                                                                                                                                                 i])
                  }
                }
              }
      ...
      if (!is.null(coord_quali) & is.na(test.invisible[3])) {
        ...
        if (graph.type == "ggplot") {
          coll_bis <- rgb(t(col2rgb(col.quali[seq_len(nrow(coord_quali))])),
                          maxColorValue = 255)
          df_quali <- data.frame(labe3, coord_quali, col.quali[seq_len(nrow(coord_quali))],
                                 rep(15, nrow(coord_quali)), rep(2, nrow(coord_quali)),
                                 coll_bis)
          gg_graph <- gg_graph + geom_point(aes(x = df_quali[,
                                                             2], y = df_quali[, 3]), color = df_quali[,
                                                                                                      7], shape = df_quali[, 5], size = ggoptions_default$size/3)
          if (autoLab)
            text_quali <- ggrepel::geom_text_repel(aes(x = df_quali[,
                                                                    2], y = df_quali[, 3], label = df_quali[,
                                                                                                            1]), size = ggoptions_default$size, color = df_quali[,
                                                                                                                                                                 7], fontface = df_quali[, 6])
          else {
            text_quali <- geom_text(aes(x = df_quali[,
                                                     2], y = df_quali[, 3], label = df_quali[,
                                                                                             1]), size = ggoptions_default$size, color = df_quali[,
                                                                                                                                                  7], hjust = (-sign(df_quali[, 2]) + 1)/2,
                                    vjust = -sign(df_quali[, 3]) * 0.75 + 0.25,
                                    fontface = df_quali[, 6])
          }
        }
        if (!is.null(partial)) {
          if (graph.type == "classic") {
            for (i in group.quali) {
              for (j in 1:nbre.grpe) {
                points(coord_quali.partiel[(i - 1) * nbre.grpe +
                                             j, 1], coord_quali.partiel[(i - 1) *
                                                                          nbre.grpe + j, 2], pch = 15, col = col.quali[nrow(coord_quali) +
                                                                                                                         (i - 1) * nbre.grpe + j], cex = par("cex") *
                         0.8)
                if (lab.var & lab.par)
                  text(coord_quali.partiel[(i - 1) * nbre.grpe +
                                             j, 1], y = coord_quali.partiel[(i -
                                                                               1) * nbre.grpe + j, 2], labels = rownames(coord_quali.partiel)[(i -
                                                                                                                                                 1) * nbre.grpe + j], pos = 3, col = col.quali[nrow(coord_quali) +
                                                                                                                                                                                                 (i - 1) * nbre.grpe + j], ...)
                if (chrono) {
                  if (j > 1) {
                    if (graph.type == "classic")
                      lines(c(coord_quali.partiel[(i -
                                                     1) * nbre.grpe + (j - 1), 1],
                              coord_quali.partiel[(i - 1) *
                                                    nbre.grpe + j, 1]), c(coord_quali.partiel[(i -
                                                                                                 1) * nbre.grpe + (j - 1), 2],
                                                                          coord_quali.partiel[(i - 1) *
                                                                                                nbre.grpe + j, 2]), col = col.quali[i])
                  }
                }
                else lines(c(coord_quali[i, 1], coord_quali.partiel[(i -
                                                                       1) * nbre.grpe + j, 1]), c(coord_quali[i,
                                                                                                              2], coord_quali.partiel[(i - 1) * nbre.grpe +
                                                                                                                                        j, 2]), col = col.quali[nrow(coord_quali) +
                                                                                                                                                                  (i - 1) * nbre.grpe + j], lty = j)
              }
            }
          }
          else {
            liste_quali_partiel <- rep(nbre.grpe * (group.quali -
                                                      1), each = nbre.grpe) + rep(1:nbre.grpe,
                                                                                  length(group.quali))
            df_quali_partial <- cbind.data.frame(matrix(rep(cbind(coord_quali[group.quali,
                                                                              , drop = FALSE], col2rgb(coll3[group.quali],
                                                                                                       alpha = TRUE)[4, ]), each = nbre.grpe),
                                                        ncol = 3), x = coord_quali.partiel[liste_quali_partiel,
                                                                                           1], y = coord_quali.partiel[liste_quali_partiel,
                                                                                                                       2], coul = col.quali[nrow(coord_quali) +
                                                                                                                                              liste_quali_partiel], group = rep(1:nbre.grpe,
                                                                                                                                                                                length(group.quali)))
            colnames(df_quali_partial)[1:3] <- c("xfin",
                                                 "yfin", "sel")
            df_quali_partial <- df_quali_partial[df_quali_partial[,
                                                                  "sel"] == 255, ]
            if (habillage == "group")
              gg_graph <- gg_graph + geom_point(data = df_quali_partial,
                                                aes(x = x, y = y, color = as.factor(coul)),
                                                shape = 15, size = ggoptions_default$size/3)
            else gg_graph <- gg_graph + geom_point(data = df_quali_partial,
                                                   aes(x = x, y = y), color = as.factor(df_quali_partial$coul),
                                                   shape = 15, size = ggoptions_default$size/3)
            if (!chrono) {
              if (habillage == "group")
                gg_graph <- gg_graph + geom_segment(aes(x = x,
                                                        y = y, xend = xfin, yend = yfin, col = as.factor(coul)),
                                                    lty = df_quali_partial$coul, data = df_quali_partial)
              gg_graph <- gg_graph + geom_segment(aes(x = x,
                                                      y = y, xend = xfin, yend = yfin), col = as.factor(df_quali_partial$coul),
                                                  lty = df_quali_partial$group, data = df_quali_partial)
            }
            else {
              for (i in group.quali) {
                if (col2rgb(coll[i], alpha = TRUE)[4] ==
                    255) {
                  for (j in 2:nbre.grpe) {
                    gg_graph <- gg_graph + geom_line(aes_string(x = c(coord_quali.partiel[(i -
                                                                                             1) * nbre.grpe + (j - 1), 1], coord_quali.partiel[(i -
                                                                                                                                                  1) * nbre.grpe + j, 1]), y = c(coord_quali.partiel[(i -
                                                                                                                                                                                                        1) * nbre.grpe + (j - 1), 2], coord_quali.partiel[(i -
                                                                                                                                                                                                                                                             1) * nbre.grpe + j, 2])), color = col.quali[nrow(coord_quali) +
                                                                                                                                                                                                                                                                                                           (i - 1) * nbre.grpe + j])
                  }
                }
              }
      ...
      if (!is.null(coord_quali_sup) & is.na(test.invisible[4])) {
        ...
        if (graph.type == "ggplot") {
          ...
          df_quali_sup <- data.frame(labe4, coord_quali_sup,
                                     coll4, ipch4, fonte4, coll_bis)
          gg_graph <- gg_graph + geom_point(aes(x = df_quali_sup[,
                                                                 2], y = df_quali_sup[, 3]), color = df_quali_sup[,
                                                                                                                  7], shape = df_quali_sup[, 5], size = ggoptions_default$size/3)
          if (autoLab)
            text_quali.sup <- ggrepel::geom_text_repel(aes(x = df_quali_sup[,
                                                                            2], y = df_quali_sup[, 3], label = df_quali_sup[,
                                                                                                                            1]), size = ggoptions_default$size, color = df_quali_sup[,
                                                                                                                                                                                     7], fontface = df_quali_sup[, 6])
          else {
            text_quali.sup <- geom_text(aes(x = df_quali_sup[,
                                                             2], y = df_quali_sup[, 3], label = df_quali_sup[,
                                                                                                             1]), size = ggoptions_default$size, color = df_quali_sup[,
                                                                                                                                                                      7], hjust = (-sign(df_quali_sup[, 2]) +
                                                                                                                                                                                     1)/2, vjust = -sign(df_quali_sup[, 3]) *
                                          0.75 + 0.25, fontface = df_quali_sup[, 6])
          }
        }
        if (!is.null(partial)) {
          if (graph.type == "classic") {
            for (i in group.quali.sup) {
              for (j in 1:nbre.grpe) {
                points(coord_quali.partiel.sup[(i - 1) *
                                                 nbre.grpe + j, 1], coord_quali.partiel.sup[(i -
                                                                                               1) * nbre.grpe + j, 2], pch = 22, col = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                       (i - 1) * nbre.grpe + j], cex = par("cex") *
                         0.8)
                if (lab.var & lab.par)
                  text(coord_quali.partiel.sup[(i - 1) *
                                                 nbre.grpe + j, 1], y = coord_quali.partiel.sup[(i -
                                                                                                   1) * nbre.grpe + j, 2], labels = rownames(coord_quali.partiel.sup)[(i -
                                                                                                                                                                         1) * nbre.grpe + j], pos = 3, col = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                                                                                             (i - 1) * nbre.grpe + j], ...)
                if (chrono) {
                  if (j > 1) {
                    if (graph.type == "classic")
                      lines(c(coord_quali.partiel.sup[(i -
                                                         1) * nbre.grpe + (j - 1), 1],
                              coord_quali.partiel.sup[(i - 1) *
                                                        nbre.grpe + j, 1]), c(coord_quali.partiel.sup[(i -
                                                                                                         1) * nbre.grpe + (j - 1), 2],
                                                                              coord_quali.partiel.sup[(i - 1) *
                                                                                                        nbre.grpe + j, 2]), col = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                  i])
                  }
                }
                else lines(c(coord_quali_sup[i, 1], coord_quali.partiel.sup[(i -
                                                                               1) * nbre.grpe + j, 1]), c(coord_quali_sup[i,
                                                                                                                          2], coord_quali.partiel.sup[(i - 1) *
                                                                                                                                                        nbre.grpe + j, 2]), col = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                                                                  (i - 1) * nbre.grpe + j], lty = j)
              }
            }
            ...
            liste_quali_sup_partiel <- rep(nbre.grpe *
                                             (group.quali.sup - 1), each = nbre.grpe) +
              rep(1:nbre.grpe, length(group.quali.sup))
            df_quali_partial_sup <- cbind.data.frame(matrix(rep(cbind(coord_quali_sup[group.quali.sup,
                                                                                      , drop = FALSE], col2rgb(coll4[group.quali.sup],
                                                                                                               alpha = TRUE)[4, ]), each = nbre.grpe),
                                                            ncol = 3), x = coord_quali.partiel.sup[liste_quali_sup_partiel,
                                                                                                   1], y = coord_quali.partiel.sup[liste_quali_sup_partiel,
                                                                                                                                   2], coul = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                              liste_quali_sup_partiel], group = rep(1:nbre.grpe,
                                                                                                                                                                                                    length(group.quali.sup)))
            colnames(df_quali_partial_sup)[1:3] <- c("xfin",
                                                     "yfin", "sel")
            df_quali_partial_sup <- df_quali_partial_sup[df_quali_partial_sup[,
                                                                              "sel"] == 255, ]
            if (habillage == "group")
              gg_graph <- gg_graph + geom_point(data = df_quali_partial_sup,
                                                aes(x = x, y = y, color = as.factor(coul)),
                                                shape = 15, size = ggoptions_default$size/3)
            else gg_graph <- gg_graph + geom_point(data = df_quali_partial_sup,
                                                   aes(x = x, y = y), col = as.factor(df_quali_partial_sup$coul),
                                                   shape = 15, size = ggoptions_default$size/3)
            if (!chrono) {
              if (habillage == "group")
                gg_graph <- gg_graph + geom_segment(data = df_quali_partial_sup,
                                                    aes(x = x, y = y, xend = xfin, yend = yfin,
                                                        color = as.factor(coul)), lty = df_quali_partial_sup$coul)
              else gg_graph <- gg_graph + geom_segment(data = df_quali_partial_sup,
                                                       aes(x = x, y = y, xend = xfin, yend = yfin),
                                                       col = as.factor(df_quali_partial_sup[,
                                                                                            "coul"]), lty = df_quali_partial_sup$group)
            }
            else {
              for (i in group.quali.sup) {
                if (col2rgb(coll[i], alpha = TRUE)[4] ==
                    255) {
                  for (j in 2:nbre.grpe) {
                    gg_graph <- gg_graph + geom_line(aes_string(x = c(coord_quali.partiel.sup[(i -
                                                                                                 1) * nbre.grpe + (j - 1), 1], coord_quali.partiel.sup[(i -
                                                                                                                                                          1) * nbre.grpe + j, 1]), y = c(coord_quali.partiel.sup[(i -
                                                                                                                                                                                                                    1) * nbre.grpe + (j - 1), 2], coord_quali.partiel.sup[(i -
                                                                                                                                                                                                                                                                             1) * nbre.grpe + j, 2])), color = col.quali.sup[nrow(coord_quali_sup) +
                                                                                                                                                                                                                                                                                                                               (i - 1) * nbre.grpe + j])
                  }
                }
              }
      ...
      if (graph.type == "ggplot") {
        gg_graph <- gg_graph + theme + text_ind + text_ind.sup +
          text_quali + text_quali.sup
        if (!is.null(partial))
          gg_graph <- gg_graph + labs(color = "")
      }
      else {
        if (shadowtext)
          points(coo[, 1], y = coo[, 2], pch = ipch, col = coll,
      ...
      if (graph.type == "ggplot" & habillage == "group" &
          !is.null(partial)) {
        gg_graph <- gg_graph + scale_color_manual(values = palette[L$text.col[order(L$text.col)]],
                                                  labels = L$legend[order(L$text.col)])
      }
  ...
  ## variables scatter plot ##################################
  reference <- FactoMineR::MFA(object[["res.imputeMFA"]],
                               group = object$call$group,
                               type = object$call$type,
                               graph = F,
                               ncp = ncomp)
  res.dim  <- object$call$X
  res.procrustes <- reference$ind$coord[, 1:ncomp]

  # Modify the loop to include num_datasets if specified
  num_imp_datasets <- ifelse(is.null(num_datasets), length(object$res.MI), num_datasets)
  for (i in 1:num_imp_datasets){
    rec.mca <- object$res.MI[[i]]
    acmfin <- FactoMineR::MFA(object$res.MI[[i]],
                              group = object$call$group,
                              type = object$call$type,
                              graph = FALSE,
                              ncp = ncomp)
    tourne <- procrustes(acmfin$ind$coord[, 1:ncomp],
                         reference$ind$coord[,1:ncomp],
                         orthogonal = TRUE,
                         translate = TRUE,
                         magnify = TRUE)$rmat
    colnames(tourne) <- colnames(res.procrustes)
    res.procrustes <- rbind.data.frame(res.procrustes,
                                       tourne)
    res.dim <- cbind.data.frame(res.dim,
                                acmfin$ind$coord[,1:ncomp])
  }

  coordinsup<-apply(object$call$tab.disj,
                    3,
                    FUN=function(tab.imp,
                                 don.na,
                                 axes,
                                 G,
                                 res.mca){
                    ...
                    don.na=object$call$X,
                    axes= comp,
                    G=object$res.tab.disj,
                    res.mca=reference)
  coordinsup<-lapply(coordinsup,"[[",1)

  colnames(res.dim) = paste("V", 1:ncol(res.dim))
  ooo = FactoMineR::MFA(cbind(object$res.imputeMFA,res.dim[,-(1:ncol(object$call$X))]),
                        num.group.sup = length(object$call$group)+1,
                        group = c(object$call$group,ncol(res.dim)-ncol(object$call$X)),
                        type = c(object$call$type,"s"),
                        graph = FALSE,
                        ncp = ncomp)
  ooo$eig = reference$eig
  if (is.null(title)) title="Projection of the Main Factors"
  PlotDim <- plotCorrected(ooo,
                           choi="var",
                           axes=comp,
                           title=title,
                           label="none",
                           ...
                           lab.var=F,
                           graph.type = "ggplot",
                           ggoptions = list(legend.position = "none")) +
